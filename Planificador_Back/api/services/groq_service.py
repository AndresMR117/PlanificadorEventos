...
class GroqService:
    ...
    _URI_A_TIPO_DJANGO = {
        ...
        'Aniversario': 'Aniversario',
        'Bautizo': 'Bautizo',
        'PrimeraComunion': 'Cumpleaños',
        'EventoCorporativo': 'EventoCorporativo',
        'Conferencia': 'EventoCorporativo',
    }
    ...
    def __init__(self):
        # ── NO se valida ni se lanza excepción aquí.
        # ── El cliente se crea de forma lazy en _get_client()
        # ── para que el módulo pueda importarse aunque la clave
        # ── aún no esté disponible en el entorno al arrancar.
        self._client = None
        self.model = 'llama-3.1-8b-instant'
        self.sparql = SPARQLService()
        print('[GROQ] GroqService inicializado (cliente lazy).')

    def _get_client(self) -> OpenAI:
        """
        Crea el cliente OpenAI/Groq la primera vez que se necesita.
        Lee la API key en el momento de la llamada, no al importar.
        """
        if self._client is None:
            api_key = os.getenv('GROQ_API_KEY')
            if not api_key:
                vars_env = [k for k in os.environ.keys()]
                print(f'[GROQ] ERROR: GROQ_API_KEY no encontrada.')
                print(f'[GROQ] Variables disponibles: {vars_env}')
                raise ValueError(
                    'GROQ_API_KEY no está configurada. '
                    'Verifica Railway → Variables.'
                )
            print(f'[GROQ] API key cargada ({api_key[:10]}...)')
            self._client = OpenAI(
                api_key=api_key,
    ...
    def _consultar_bd_semantica(self, tipo_uri: str, presupuesto: int | None,
        ...
        if tipo_uri:
            ...
            if presupuesto:
                dentro = [s for s in servicios if float(s.get('precioBase') or 0) <= presupuesto]
                fuera  = [s for s in servicios if float(s.get('precioBase') or 0) >  presupuesto]
        ...
        if ciudad:
            ...
            resultado['proveedores'] = provs if provs else self.sparql.obtener_proveedores_destacados()
        else:
    ...
    def _formatear_bd_para_prompt(self, bd: dict, tipo_label: str,
        ...
        if bd.get('servicios_fuera'):
            lineas.append('\n⚠️ SERVICIOS QUE SUPERAN EL PRESUPUESTO DEL CLIENTE (NO recomendar):')
        ...
        if bd['proveedores']:
            ...
            for p in bd['proveedores']:
                ...
                linea    = f'  • {nombre}'
                if cal:      linea += f' ⭐{cal}/5'
                if ciudad_p: linea += f' | {ciudad_p}'
                if tel:      linea += f' | Tel: {tel}'
                lineas.append(linea)
    ...
    def generar_respuesta(self, mensaje: str, contexto: str = None,
        ...
        print(f'[GROQ] Estado → tipo={tipo_uri}, ppto={presupuesto}, '
              f'personas={personas}, ciudad={ciudad}, fecha={fecha}')
        ...
        recopilados, pendientes = [], []
        if tipo_label:   recopilados.append(f'✅ Tipo de evento: {tipo_label}')
        else:            pendientes.append('❌ Tipo de evento')
        if presupuesto:  recopilados.append(f'✅ Presupuesto: ${presupuesto:,.0f} COP')
        else:            pendientes.append('❌ Presupuesto')
        if personas:     recopilados.append(f'✅ Número de personas: {personas}')
        else:            pendientes.append('❌ Número de personas')
        if ciudad:       recopilados.append(f'✅ Ciudad: {ciudad}')
        else:            pendientes.append('❌ Ciudad')
        if fecha:        recopilados.append(f'✅ Fecha: {fecha}')
        else:            pendientes.append('❌ Fecha')
...
3. TIENES TIPO + PRESUPUESTO → CONVERSACIÓN LIBRE:
   ...
   - Si el cliente pregunta por un servicio específico, respóndele directamente.
   ...
   - COMPARACIÓN DE PRESUPUESTO (REGLA CRÍTICA):
...
4. REGLAS GENERALES:
   ...
   - CORRECCIONES DEL USUARIO: acéptalas con gracia y continúa ayudando.
   - NUNCA generes respuestas de rechazo o bloqueo.
   - HISTORIAL = CONVERSACIÓN ACTUAL solamente.

5. CUANDO TENGAS LOS 5 DATOS (tipo, presupuesto, personas, ciudad, fecha):
   Muestra el resumen final con este formato exacto:
...
✅ Haz clic en **Guardar Evento** cuando quieras confirmar."""
        ...
        try:
            ...
            # _get_client() lee la key en el momento de la llamada
